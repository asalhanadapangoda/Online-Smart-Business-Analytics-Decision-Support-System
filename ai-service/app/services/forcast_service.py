...
try:
    from prophet import Prophet  # type: ignore # pylint: disable=import-error
    HAS_PROPHET = True
except ImportError:
    HAS_PROPHET = False
    Prophet = None  # Define in global scope to avoid name lookup errors
...
def generate_forecast(historical_data: List[dict], horizon_days: int) -> dict:
    ...
    if HAS_PROPHET:
        try:
            ...
            logger.warning(f"Prophet execution failed: {e}. Using moving average fallback.")
            return _moving_average_forecast(historical_data, horizon_days)
    else:
        logger.warning("Prophet package is not installed. Using moving average fallback.")
        return _moving_average_forecast(historical_data, horizon_days)
